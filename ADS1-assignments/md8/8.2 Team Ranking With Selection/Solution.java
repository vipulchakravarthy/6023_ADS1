...
class Team implements Comparable<Team> {
     ...
     */
    public int compareTo(final Team teamTwo) {
        if (this.getWins() < teamTwo.getWins()) {
            return  -1;
        } else if (this.getWins() > teamTwo.getWins()) {
            return 1;
        } else {
            if (this.getLosses() < teamTwo.getLosses()) {
                return 1;
            } else if (this.getLosses() > teamTwo.getLosses()) {
                return -1;
            } else {
                if (this.getDraw() < teamTwo.getDraw()) {
                    return -1;
                } else if (this.getDraw() > teamTwo.getDraw()) {
                    return 1;
                }
            }
        }
    ...
    * list is the arraylist created.
    ...
    public void sorting() {
        ...
        for (int i = 0; i < list.size(); i++) {
            for (int j = i; j > 0; j--) {
                flag = list.get(j - 1).compareTo(list.get(j));
                if (flag == -1) {
